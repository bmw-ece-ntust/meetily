...
/// Now always returns true since transcription is handled by ai-meeting-agent API
async fn check_can_record<R: Runtime>(_app: &AppHandle<R>) -> bool {
    // Always allow recording - transcription happens via ai-meeting-agent API
...
fn build_menu<R: Runtime>(
    ...
    _can_record: bool, // Unused - recording is now always available (uses ai-meeting-agent API)
) -> tauri::Result<tauri::menu::Menu<R>> {
    let mut builder = MenuBuilder::new(app);

    // Recording is now always available (uses ai-meeting-agent API)
    match state {
