    ...
    pub fn new(
        ...
        let (noise_suppressor, high_pass_filter, normalizer) = if matches!(device_type, DeviceType::Microphone) {
            ...
            let ns = if super::ffmpeg_mixer::RNNOISE_APPLY_ENABLED {
                ...
                info!("   API transcription handles noise well internally - RNNoise is optional");
                None
            };
...
/// Uses Voice Activity Detection to segment speech in real-time and send only speech to transcription API
...
impl AudioPipeline {
    pub fn new(
        ...
        let vad_processor = match ContinuousVadProcessor::new(sample_rate, redemption_time) {
            Ok(processor) => {
                info!("VAD-driven pipeline: VAD segments will be sent directly to transcription API (no time-based accumulation)");
                processor
            }
            Err(e) => {
                ...
                Ok(Some(chunk)) => {
                    ...
                    while self.ring_buffer.can_mix() {
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            // STEP 3: Send mixed audio for transcription (VAD + API)
                            match self.vad_processor.process_audio(&mixed_with_gain) {
                                Ok(speech_segments) => {
                                    for segment in speech_segments {
                                        let duration_ms = segment.end_timestamp_ms - segment.start_timestamp_ms;

                                        if segment.samples.len() >= 800 {  // Minimum 50ms at 16kHz - API requirement
            ...
            Ok(final_segments) => {
                for segment in final_segments {
                    ...
                    if segment.samples.len() >= 800 {
                        info!("📤 Sending final VAD segment to transcription API: {:.1}ms duration, {} samples",
                              duration_ms, segment.samples.len());
