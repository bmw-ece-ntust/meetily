...
pub async fn start_retranscription<R: Runtime>(
    ...
    // Cloud-first: no local engine to unload
...
async fn run_retranscription<R: Runtime>(
    ...
    // Initialize the cloud transcription engine once (Phase 3+: no local engine variants)
    let transcription_engine = crate::audio::transcription::get_or_init_transcription_engine(&app)
        .await
